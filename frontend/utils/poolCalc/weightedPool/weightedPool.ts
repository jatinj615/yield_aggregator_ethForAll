...
import { BigNumber as OldBigNumber, bnum, scale, ZERO } from '../utils/bignumber';
...
export class WeightedPool implements PoolBase {
  ...
  _exactTokenInForTokenOut(poolPairData: WeightedPoolPairData, amount: OldBigNumber, exact: boolean): OldBigNumber {
    if (exact) {
      try {
    ...
    return _exactTokenInForTokenOut(amount, poolPairData).dp(poolPairData.decimalsOut, 1);
  }

  // Using BigNumber.js decimalPlaces (dp), allows us to consider token decimal accuracy correctly,
  // i.e. when using token with 2decimals 0.002 should be returned as 0
  // Uses ROUND_UP mode (0)
  _tokenInForExactTokenOut(poolPairData: WeightedPoolPairData, amount: OldBigNumber, exact: boolean): OldBigNumber {
    if (exact) {
      try {
        // poolPair balances are normalised so must be scaled before use
    ...
    return _tokenInForExactTokenOut(amount, poolPairData).dp(poolPairData.decimalsIn, 0);
  }
